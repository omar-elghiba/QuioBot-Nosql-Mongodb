...
async def add_student_data(customer: CustomerSchema = Body(...)):
...
@router.put("/{id}")
async def update_customer_data(id: str, req: UpdateCustomer = Body(...)):
...
async def delete_customer_data(id: str):
    ...
    if deleted_customer:
        return ResponseModel(
            "Student with ID: {} removed".format(id), "customer deleted successfully"
        )
    return ErrorResponseModel(
